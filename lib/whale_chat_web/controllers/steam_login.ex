defmodule WhaleChatWeb.SteamLogin do
  @moduledoc false
  ...
  defp login_or_callback(conn, %{"steamid" => steamid}, return_to, opts) when is_binary(steamid) do
  ...
  defp absolute_login_return_url(conn, return_to, opts) do
    request_realm(conn) <> Keyword.fetch!(opts, :login_path) <> "?return=" <> URI.encode(return_to)
  end
  ...
  defp render_error_page(conn, return_to, error_msg, opts) do
    ...
    session_detail = Keyword.get(opts, :session_detail, fn _steamid, _esc -> "" end).(steamid, esc)
